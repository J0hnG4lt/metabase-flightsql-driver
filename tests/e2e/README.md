...
| `test_dremio.py` | Dremio (OSS) lakehouse — the suite's **Apache Iceberg** proof: user/pass auth + wrong-password reject, **PAT/Bearer** auth (only real server besides InfluxDB), reads an Iceberg table, multi-schema sync + adaptive schema filter, CTAS-Iceberg write round-trip. Single-node Dremio serves Flight results inline so it runs anywhere; auto-skips via `DREMIO_READY` |
...
| `test_kamu.py` | kamu / Open Data Fabric — the suite's **bitemporal / verifiable** proof: anonymous auth, ODF system columns (`offset`/`op`/`system_time`/`event_time`) synced, the `op` changelog (append/retract/correct), `to_table()` current-state collapse, and time-travel reconstruction as of a past transaction time. DataFusion over Flight SQL, read-only; shares one connection (kamu is slow under connection churn). Needs the kamu profile — auto-skips via the `:50050` port |
